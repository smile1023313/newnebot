...
def ranpage():
    ...
    for page in range(start_page, start_page - page_term, -1):
        page_url = 'https://www.ptt.cc/bbs/Beauty/index{}.html'.format(page)
        index_list.append(page_url)
    rannum = random.randint(1,len(index_list)-1)
    rs = requests.session()
    res = rs.get(index_list[rannum], verify=False)#res.text
    soup = BeautifulSoup(res.text, 'html.parser')
    titles = soup.find_all('a') 
    pic_page_urls = []
    str2 = '/bbs/Beauty/M.'
    for title in titles:#隨機選第幾頁後的那頁的全部超連結
        if title.get('href').find(str2,0,15) != -1: #從全部的超連結中，選出正確的網址
            pic_page_url = 'https://www.ptt.cc'+title.get('href') #加上主網址
            pic_page_urls.append(pic_page_url)
    rannum = random.randint(0,len(pic_page_urls)-1)
    rs = requests.session()
    res = rs.get(pic_page_urls[rannum], verify=False)
    soup = BeautifulSoup(res.text, 'html.parser')
    titles = soup.find_all('a') 
    img_urls = []
    str2 = 'https://i.imgur.com/'
    for title in titles:#隨機選第幾篇文後的那文章的全部超連結
        if title.get('href').find(str2,0,20) != -1: #從全部的超連結中，選出正確的圖片
            pic_page_url = title.get('href') 
            img_urls.append(pic_page_url)
    rannum = random.randint(0,len(img_urls)-1)
    return img_urls[rannum]
...
def handle_message(event):
    ...
    if event.message.text == "PanX泛科技":
        ...
        line_bot_api.reply_message(
            ...
            TextSendMessage(text=content))
        return 0
    if event.message.text == "扭捏":
        buttons_template = TemplateSendMessage(
            alt_text='扭捏 template',
